...
class InterfacePlateau:
    def __init__(self, parent):
        ...
        self.window.title("Theoretical Number of Plates")
        ...
        tk.Label(header, text="Theoretical Number of Plates",
                 font=("Segoe UI", 20, "bold"), fg=TEXT_PRI, bg=BG).pack(anchor="w", pady=(2, 1))
        tk.Label(header, text="N = 16 (tR / wb)² via retention time and peak width",
        ...
        tk.Label(card2,
                 text="Paste two columns: retention time  |  peak width base\n"
    ...
    def _calculate_manual(self):
        try:
            ...
            self.label_manual_result.config(text=f"N  =  {res:.2f} plates", fg=SUCCESS)
            self.label_manual_error.config(text="")
        except ValueError:
            self.label_manual_error.config(text="Please enter valid numbers in both fields.")
    ...
    def _calculate_row(self):
        ...
        try:
            res = theorical_plates_one(tR, wb)
            self.label_result.config(text=f"N = {res:.2f} plates")
