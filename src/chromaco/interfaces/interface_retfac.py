...
class InterfaceRetentionFactor:
    def __init__(self, parent):
        ...
        self.fenetre.title("Retention Factor k")
        ...
        tk.Label(hdr, text="Retention Factor k",
                 font=("Segoe UI", 20, "bold"), fg=TEXT_PRI, bg=BG).pack(anchor="w", pady=(2,1))
        tk.Label(hdr, text="k = (tR − t0) / t0",
        ...
        self.entree1 = _labelled_entry_row(card, "Retention time tR (min)")
        self.entree2 = _labelled_entry_row(card, "Dead time t0 (min)")
        ...
        _action_btn(sf, "  Calculate Retention Factor", self._calculer_retention, SUCCESS, large=True).pack(pady=(4, 0))
    ...
    def _calculer_manuel(self):
        try:
            ...
            t0 = float(self.entree2.get().replace(",", "."))
    ...
    def _calculer_retention(self):
        ...
        if len(sel) != 1:
            messagebox.showwarning("Select 1 row", "Please select exactly 1 row.", parent=self.fenetre)
            return
        ...
        try:
            k = calculate_retention_factor(tR, t0)
            self.label_resultat.config(text=f"k = {k:.4f}\n(Row {idx+1})", fg=SUCCESS)
