...
class TestCraigBatterySimulation(unittest.TestCase):

    # ── Basic correctness ────────────────────────────────────────────────────

    def test_returns_dict_of_arrays(self):
        history = craig_battery_simulation(10, 5, {"A": 1.0})
        self.assertIn("A", history)
        self.assertIsInstance(history["A"], np.ndarray)

    def test_array_shape(self):
        history = craig_battery_simulation(10, 5, {"A": 1.0})
        self.assertEqual(history["A"].shape, (5, 10))

    def test_mass_conservation(self):
        """Total mass across all tubes must remain 1.0 at every step."""
        history = craig_battery_simulation(20, 10, {"X": 2.0}, {"X": 1.0})
        for step in range(10):
            self.assertAlmostEqual(history["X"][step].sum(), 1.0, places=10)

    def test_mass_conservation_multiple_compounds(self):
        history = craig_battery_simulation(
            30, 15,
            {"A": 1.0, "B": 3.0},
            {"A": 1.0, "B": 1.0},
        )
        for name in ("A", "B"):
            for step in range(15):
                self.assertAlmostEqual(
                    history[name][step].sum(), 1.0, places=10,
                    msg=f"Mass not conserved for {name} at step {step}",
                )

    def test_peak_advances_with_steps(self):
        """Higher K → slower migration; peak position should be lower."""
        history_slow = craig_battery_simulation(50, 20, {"slow": 5.0})
        history_fast = craig_battery_simulation(50, 20, {"fast": 0.5})
        peak_slow = np.argmax(history_slow["slow"][-1])
        peak_fast = np.argmax(history_fast["fast"][-1])
        self.assertLess(peak_slow, peak_fast)

    def test_two_compounds_separate(self):
        """Compounds with very different K values must have non-overlapping peaks."""
        history = craig_battery_simulation(
            100, 40,
            {"polar": 9.0, "apolar": 1.0},
        )
        peak_polar  = np.argmax(history["polar"][-1])
        peak_apolar = np.argmax(history["apolar"][-1])
        self.assertNotEqual(peak_polar, peak_apolar)

    def test_initial_load_respected(self):
        """Step 0 must equal the initial load in tube 0."""
        history = craig_battery_simulation(10, 5, {"A": 1.0}, {"A": 2.5})
        self.assertAlmostEqual(history["A"][0, 0], 2.5, places=10)

    def test_default_initial_load_is_one(self):
        history = craig_battery_simulation(10, 5, {"A": 1.0})
        self.assertAlmostEqual(history["A"][0, 0], 1.0, places=10)

    def test_multiple_compounds_independent(self):
        """Running compounds together vs. separately must give identical histories."""
        h_joint = craig_battery_simulation(20, 8, {"A": 1.0, "B": 3.0})
        h_A     = craig_battery_simulation(20, 8, {"A": 1.0})
        h_B     = craig_battery_simulation(20, 8, {"B": 3.0})
        np.testing.assert_array_almost_equal(h_joint["A"], h_A["A"])
        np.testing.assert_array_almost_equal(h_joint["B"], h_B["B"])

    # ── Input validation ─────────────────────────────────────────────────────

    def test_n_tubes_zero_raises(self):
        with self.assertRaises(ValueError):
            craig_battery_simulation(0, 5, {"A": 1.0})

    def test_n_tubes_negative_raises(self):
        with self.assertRaises(ValueError):
            craig_battery_simulation(-5, 5, {"A": 1.0})

    def test_n_tubes_float_raises(self):
        with self.assertRaises(ValueError):
            craig_battery_simulation(10.5, 5, {"A": 1.0})

    def test_n_steps_zero_raises(self):
        with self.assertRaises(ValueError):
            craig_battery_simulation(10, 0, {"A": 1.0})

    def test_n_steps_negative_raises(self):
        with self.assertRaises(ValueError):
            craig_battery_simulation(10, -3, {"A": 1.0})

    def test_n_steps_exceeds_n_tubes_raises(self):
        with self.assertRaises(ValueError):
            craig_battery_simulation(5, 10, {"A": 1.0})

    def test_empty_k_values_raises(self):
        with self.assertRaises(ValueError):
            craig_battery_simulation(10, 5, {})

    def test_negative_k_raises(self):
        with self.assertRaises(ValueError):
            craig_battery_simulation(10, 5, {"A": -1.0})

    def test_zero_k_raises(self):
        with self.assertRaises(ValueError):
            craig_battery_simulation(10, 5, {"A": 0.0})

    def test_mismatched_initial_load_keys_raises(self):
        with self.assertRaises(ValueError):
            craig_battery_simulation(10, 5, {"A": 1.0}, {"B": 1.0})

    def test_zero_initial_load_raises(self):
        with self.assertRaises(ValueError):
            craig_battery_simulation(10, 5, {"A": 1.0}, {"A": 0.0})

    def test_negative_initial_load_raises(self):
        with self.assertRaises(ValueError):
            craig_battery_simulation(10, 5, {"A": 1.0}, {"A": -1.0})

    def test_invalid_visual_interval_raises(self):
        with self.assertRaises(ValueError):
            craig_battery_simulation(10, 5, {"A": 1.0}, visual_interval=0)
